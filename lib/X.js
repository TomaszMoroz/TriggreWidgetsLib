  ...
  static async init(target, callback) {
    try {
      ...
      for (const el of widgets) {
        ...
        el.classList.remove('hidden'); // Remove hidden class
    ...
    for (const el of widgets) {
      ...
      if (widgetInstance) {
        ...
        setTimeout(() => {
          ...
          el.classList.remove('visible', 'bounce-in', 'fade-out', `initialized-${widgetName}`);
          
          // Clear the inner HTML and hide the widget
          el.innerHTML = DOMPurify.sanitize(''); // Sanitize and clear inner content
        ...
        }, 400); // Match this with the duration of your fade-out animation
      }
    }
  }
